...
for i = 1:size(sv,2) % 1:number of dives
    ...
    % Get value(s) at or above -22 dB intensity and deeper than 35 m
    max_vals = find(xmax >= -23); %Within vector indices for values that are above -23 dB
    max_deps = imax(max_vals); % Depth index values that have intensites greater than -23 dB
    max_depinds = find(max_deps >= 0); % Indices in the max deps that are above 70 bins
    max_deps = max_deps(max_depinds); % Reassign the variable so that it only retains the depths that are above 35 m and that are above -23 dB
    ...
    elseif length(max_deps) > 1 % If there are more than one value that could be the bottom... 
        max_ind = min(max_deps); % The shallowest value from among those that are greater than -23 dB intensity and that is deeper than 35 m
        bott_sv(i) = sv(max_ind,i);
        bott_dep(i) = depth(max_ind,i);
        bott_ind(i) = max_ind;
    end
