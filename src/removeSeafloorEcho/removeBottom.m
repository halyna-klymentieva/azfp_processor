...
% 
% bott_sv1 = bott_sv(268:1490);
% bott_dep1 = bott_dep(268:1490);
% bott_ind1 = bott_ind(268:1490); % These are the row indices for each dive
% 
...
% New transit 1 matrices with only the data above the bottom included:
for i = 1:length(bott_sv)
    if ~isnan(bott_ind(i))
    tempdata = sv(1:bott_ind(i)-7,i); % Take all data from 3 m (= 6 bins) above the bottom bin
